    ...
    def __call__(self, geom_dict, **kwargs):
        ...
        **kwargs
        ...
        if string_weights is not None:
            ...
            repulsion = normalized_repulsion.mean() #* string_probs.sum()
        else:
            repulsion_matrix = radius_weight / (dist_sq + min_dist)
            repulsion = torch.sum(repulsion_matrix) / n
...
class ROVPenalty(LossFunction):
    ...
    def __call__(self, geom_dict, **kwargs):
        ...
        angle_softmin_tau = float(kwargs.get('rov_angle_softmin_tau', 0.01))
        ...
        least_blocked_angle_idx_per_string = angle_scores_per_angle.argmin(dim=1)  # (N,)
