...
for geom_name in ['compact', 'default', 'expanded', 'modified', 'large', '102geom', '160geom']:
    ...
    # events_file_name = f'res_test_signal_events_10000_r800_1'
    ...
    for k, event in enumerate(signal_events):
        for key in event:
            if isinstance(event[key], torch.Tensor):
                signal_events[k][key] = event[key].cpu()
